...
export class Profiles extends DatabaseService {
  ...
  async find(params?: Params): Promise<any> {
    ...
    const domains= await this.findDataToArray(config.dbCollections.domains,{ query:{id: { $in: domainIds }}});
  ...
  async get(id: Id, params: Params): Promise<any> {
    ...
    if(IsNotNullOrEmpty(account)){   
      const domains = await this.findDataToArray(config.dbCollections.domains,{ id: { $eq: account.locationDomainId } });
      let domainModel: any;
