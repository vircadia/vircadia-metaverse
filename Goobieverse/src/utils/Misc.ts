...
export function IsNullOrEmpty(pVal: any): boolean {return (typeof(pVal) === 'undefined')|| (pVal === null)|| (typeof(pVal) === 'string' && String(pVal).length === 0);}

// Return 'true' if the passed value is not null or empty
export function IsNotNullOrEmpty(pVal: any): boolean {return !IsNullOrEmpty(pVal);}
  ...
  if (pFilenameOrURL.startsWith('http://')) {
    ...
    if (pFilenameOrURL.startsWith('https://')) {
      ...
      try {
        ...
        configBody = '';
        console.debug(`readInJSON: failed read of user config file ${pFilenameOrURL}: ${err}`);
      }
    }
  }
...
export async function getMyExternalIPAddress(): Promise<string> {
  ...
  return new Promise(( resolve, reject) => {
    httpsRequest('https://api.ipify.org').then( resp => {
      myExternalAddr = resp;
