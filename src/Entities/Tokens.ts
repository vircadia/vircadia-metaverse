...
export const Tokens = {
  ...
  async getTokenWithTokenId(pTokenId: string): Promise<AuthToken> {
    return IsNullOrEmpty(pTokenId) ? null : getObject(tokenCollection, { 'id': pTokenId });
  },
  ...
  async removeToken(pAuthToken: AuthToken) : Promise<boolean> {
    return deleteOne(tokenCollection, { 'id': pAuthToken.id });
  },
  async updateTokenFields(pEntity: AuthToken, pFields: VKeyedCollection): Promise<AuthToken> {
    return updateObjectFields(tokenCollection, { 'id': pEntity.id }, pFields);
  },
